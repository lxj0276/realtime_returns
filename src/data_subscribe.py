...
from new_thread.NewThread import NewThread
from src.raw_holding_process import RawHoldingStocks
...
def data_subscribe(source):
    ...
    if source=='wind':
        ...
        w.wsq(','.join(underlyings),','.join(gv.POOL_COLUMNS),func = wind_callback)
        ...
        for tick in ticks:
            ...
            gv.UNDL_POOL_INFO[RawHoldingStocks.addfix(tick.sec_id).addfix(tick.sec_id)] = tempdata
        # 加入线程
            ...
            for undl in holdings:
                if undl not in gv.UNDL_POOL_INFO:
                    ...
                    if pathtype == 'Geometric':
                        ...
                        gv.UNDL_POOL_INFO[undl] *= np.exp(trend*step + sigma*np.sqrt(step)*np.random.randn(1,colnum))[0]
                    else:
                        gv.UNDL_POOL_INFO[undl] += np.sqrt(step) * np.random.randn(1,colnum)[0]
